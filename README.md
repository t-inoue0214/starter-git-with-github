...
1. このリポジトリをフォークするため、右上の`fork`をクリックする（Forkの意味については [00. 歴史と概念](./00_history_and_concept/README.md#7-コラムforkフォークとは) で解説しています）
...
| :--- | :--- |
| **Git** | 2.39 以上推奨 |
| **GitHub** | アカウント必須（無料プランで可） |
| **GitHub Codespaces** | ブラウザのみで使用可。ローカルインストール不要 |
...
| **05** | [現場で役立つ便利機能](./05_useful_tips/README.md) | `.gitignore`, `stash` / `diff` オプション / `worktree` / 調査テクニック |
| **06** | [実践シナリオ](./06_practical_scenarios/README.md) | GitHub Flow の流れ / 安全な pull / コミット整理 / 間違い修正 / 調査 |
